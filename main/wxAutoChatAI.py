...
"""
    auther: X.D.Yang(yxd179@gmail.com)
    descrip:
...
# linux执行登陆请调用下面的这句
bot = Bot(console_qr=False, cache_path="wxpy.pkl")
...
class AutoAI:
    ...
    def __init__(self):
        cp = ConfigParser()
        cp.read("resources/conf/setting.conf", encoding='UTF-8')
    ...
    def auto_ai(self, text):
        ...
        if auto:
            if 'url' in result["results"][0]["values"].keys():
                return u"[谁与大人正忙碌中，由啵啵AI为您服务] " + result["results"][1]["values"]["text"] + result["results"][0]["values"]["url"]
            else:
                return u"[谁与大人正忙碌中，由啵啵AI为您服务] " + result["results"][0]["values"]["text"]
        else:
            if 'url' in result["results"][0]["values"].keys():
    ...
    def getHelloWord(self):
        ...
        hello_list.append(u"在没？")
        hello_list.append(u"唠嗑了？")
    ...
    def return_bengkui(self, msg):
        ...
        hello_list.append(u"啊?")
        hello_list.append(u"what")
        if msg in hello_list:
            return u'啵啵AI大大撩错啦 就问你崩溃不 [奸笑]'
